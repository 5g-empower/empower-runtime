...
""" LNS Protocol Server.
    Handles the WS connection with a lGTW
"""
...
class LNSPMainHandler(tornado.websocket.WebSocketHandler):
    ...
    def check_origin(self, origin):
        ...
        This is a security protection against cross site scripting attacks on browsers, 
        since WebSockets are allowed to bypass the usual same-origin policies and don’t use CORS headers.
        This is an important security measure; don’t disable it without understanding the security implications. 
        In particular, if your authentication is cookie-based, you must either restrict the origins allowed 
        by check_origin() or implement your own XSRF-like protection for websocket connections. 
    ...
    def on_close(self):
        ...
        try:
            ...
            self.lgtw.last_seen    = datetime.fromtimestamp(self.lgtw.last_seen_ts).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
            self.lgtw.save() ##REVIEW test
    ...
    def on_message(self, raw_msg):
        ...
        try:
            ...
            LOG.error("Invalid input: %s", raw_msg)
        except KeyError:
            LOG.error("Invalid input: %s", raw_msg)
        except:
            raise
            ...
            if msgtype not in lnsp.UP_PACKET_TYPES:
                ...
                return
            LOG.info("Got a %s message from LGTW %s", msgtype, str(self.lgtw.lgtw_euid))
            """If lGTW is not online, check if it is a version or new state msg """
            if msgtype not in [lnsp.PT_LGTW_VERSION] and not self.lgtw.is_online():
                LOG.info("Got %s message from non connected LGTW %s, dropping message", msgtype, str(self.lgtw.lgtw_euid))
                return
    ...
    def _handle_rtt_data_rx(self, RefTime, rxtime):
        ...
        if self.rtt_on:  
            """ Call registered callbacks """
            self.call_registered_callbacks(lnsp.RTT_RX, RefTime=RefTime, MuxTime=self.last_rtt_time, rxtime=rxtime)

    """ Handle uplink messages from lGTW """  
    def _handle_version(self, lgtw_version, rxtime):
        ...
        if self.lgtw.state != lgtwState.ONLINE:
            try:
                ...
                LOG.info("Malformmed version message from LoRaWAN Radio GTW (%s)", str(self.lgtw.lgtw_euid))
                LOG.info(err)
                ...
                if "features" in lgtw_version:
                    ...
                    self.lgtw.rmtsh = lgtw_version["features"].find("rmtsh") > 0
                ...
                """ Call registered callbacks """
                self.call_registered_callbacks(lnsp.PT_LGTW_VERSION, lgtw_version=lgtw_version, rxtime=rxtime)
    ...
    def _handle_dntxed(self, dntxed_msg, rxtime):
        ...
        data['dntxed'] = {}
        try:
            data['dntxed']["diid"]     = dntxed_msg["diid"]   # copy of the field diid of the dnmsg message.
            data['dntxed']["DevEui"]   = dntxed_msg["DevEui"]
            data['dntxed']["rctx"]     = dntxed_msg["rctx"]   # specifies the antenna used for transmit 
            data['dntxed']["xtime"]    = dntxed_msg["xtime"]  # exact internal time when the frame was put on air.
    ...
    def _handle_rmtsh(self, rmtsh_msg, rxtime):
        ...
        """  Station response:  
        message describing the current state of all sessions:
        reply_msg = {
                      "msgtype"  : "rmtsh",
                      "rmtsh"    : [
                        {
        ...
        """ Call registered callbacks """
        self.call_registered_callbacks(lnsp.PT_RMT_SH, rmtsh_status=rmtsh_status, rxtime=rxtime)
        
    """ Handle uplink messages from lEndDev """                  
    def _handle_radio_data(self, rx_msg, rxtime):
        ...
        data = {}
        data['radio_data'] = {}
        data['radio_data']['DR']     = rx_msg['DR']   # data rate the frame was received on [1..15]
        data['radio_data']['Freq']   = rx_msg['Freq'] # receive frequency in Hz
        ...
        data['radio_data']["xtime"]  = rx_msg['upinfo']["xtime"]  # internal time of the lGTW 
        data['radio_data']["rctx"]   = rx_msg['upinfo']["rctx"]   # specifies the antenna used
        data['radio_data']["radio_unit"] = (rx_msg['upinfo']["rctx"]  >> 56) &  0x7F  # 
        # Encoding of xtime:
        #  bits:
        ...
        # data["xtime"]   = rx_msg['upinfo']["xtime"] & 0xFFFFFFFF  #TOCHECK 32bit
        #
        ...
        #   6-0   radio unit
        #
        # data["rctx"]    = rx_msg['upinfo']["rctx"]  #  specifies the antenna used for transmit/rx
        #
        # In UDP, for txpk, the antenna is simply the ant field. It comes from the multiple rsig entries, where it's also ant
        #
        # gpstime is the receive timetamp of the frame expressed in microseconds since GPS epoch. 
        # If Station does not have access to a PPS signal synchronized to GPS time, the value of this field is 0.
        #
        ...
        # self.lgtw.radio_data.append(radio_data)
    ...
    def _handle_jreq(self, join_msg, rxtime):
        ...
        try:
            ...
            data["join_data"]["JoinEui"]  = join_msg["JoinEui"].replace("-","").replace(":","")
            data["join_data"]["DevEui"]   = join_msg["DevEui"].replace("-","").replace(":","")
            data["join_data"]["DevNonce"] = join_msg["DevNonce"]
            data["join_data"]["MIC"]      = join_msg["MIC"] & (2**32 -1) ##TOCHECK
            data["xtime"]    = join_msg['upinfo']["xtime"] # internal time of the lGTW 
            ...
            data["PhyPayload"] = struct.pack("<BQQHI", join_msg["MHdr"], 
                                                int(data["join_data"]["JoinEui"], 16), int(data["join_data"]["DevEui"], 16), 
                                                join_msg["DevNonce"] & 0xFFFF, join_msg["MIC"] & (2**32 -1)).hex() ##TODO Check Types
                            
            if data["join_data"]["DevEui"] in self.server.lenddevs:
                LOG.info(self.server.lenddevs[data["join_data"]["DevEui"]].joinEUI) ##TOCHECK
            else:
                LOG.info("Device " + data["join_data"]["DevEui"] + " is not in the database")
            """ Call registered callbacks """
            data["msg"] = join_msg
    ...
    def _handle_updf(self, updf_msg, rxtime):
        ...
        try:
            ...
            data["updf_data"] ["DevAddr"]          = updf_msg["DevAddr"] & (2**32 -1) ##TOCHECK
            ...
            data["updf_data"] ["MIC"]              = updf_msg["MIC"] & (2**32 -1) ##TOCHECK
            data["xtime"]    = updf_msg['upinfo']["xtime"] # internal time of the lGTW 
            ...
            """ Call registered callbacks """
            fopts      = bytes.fromhex(updf_msg['FOpts'] if updf_msg['FOpts'] else '')
            fport      = bytes.fromhex('%02x' % updf_msg['FPort']  if updf_msg['FPort'] >= 0 else '')
            frmpayload = bytes.fromhex(updf_msg['FRMPayload'] if updf_msg['FRMPayload'] else '')
            data["PhyPayload"]  = struct.pack("<BIBBB{}s{}s{}sI".format(len(fopts), len(fport), len(frmpayload)),
                                                 updf_msg['MHdr'], updf_msg['DevAddr'], 
                                                 updf_msg['FCtrl'] & 0xFF, updf_msg['FCnt'] & 0xFF, (updf_msg['FCnt']>>8) & 0xFF, 
                                                 fopts, fport, frmpayload, updf_msg['MIC'] & (2**32 -1)).hex()
    ...
    def send_message(self, msgtype, msg):
        ...
        if msgtype in lnsp.DN_PACKET_TYPES: 
            ...
            if self.lgtw.is_send_rtt_on():
                ...
                # seconds and marks the time when this message is sent by the LNS
            ...
            if self.lgtw.is_send_rtt_on():
                ...
                """ call RTT callbacks  """
                self.call_registered_callbacks(lnsp.RTT_TX, muxtime=self.last_rtt_time)
            """ call MSG SENT callbacks  """
            self.call_registered_callbacks(msgtype, msg=msg, tx_time=tx_time)
        
    """ Send downlink message to lEndDev """        
    def send_lgtw_downlink_frame(self, dev_eui, pdu, params):
        """Send Downlink Frame to lGTW."""
        ##TODO check dn_frame format (Class A/B/C device)
        ##TODO check MAC Commands, Port etc.
        ## Class A
        ...
        try:
            msg = {
                  "DevEui"   : dev_eui,
                  "dC"       : 0,                  # class A
                  "diid"     : params["diid"],     # INT64, a number issued by the LNS to identify a particular device interaction.
                  "pdu"      : pdu          ,      # "HEX", radio frame as it will be put on air.
                  ...
                  "priority" : params["priority"], # INT(0..255),  # tx parameters
                  "xtime"    : params["xtime"],    # INT64, #values that originate from the corresponding uplink message
                  "rctx"     : params["rctx"],     # INT64  #values that originate from the corresponding uplink message
                }
        finally:
            ##TODO Class B & C
            ##TODO Check how to select tx parameters        
    ...
    def send_lgtw_dnschede(self, schedule):
        ...
        This type of transmission does not generate dntxed responses from Station.
        ...
        The field rctx is OPTIONAL but MAY be used to select an antenna preference. 
        LNS MAY copy this field from a previous Class A uplink and 
        guide Station to use the same antenna which received that Class A uplink.
        """
        ##CC TO REVIEW
        ##TODO check schedule format
        msg = {}
        msg['schedule']=schedule
        self.send_message(lnsp.PT_DN_SCHED, msg)
    
    """ Send downlink messages to lGTW """
    def send_lgtw_config(self):
        """ call callbacks  """
        self.call_registered_callbacks(lnsp.PT_DN_MSG, config=self.lgtw.lgtw_config)
    ...
    def send_lgtw_timesync_GPS_time(self, xtime, gpstime):
        """ call callbacks  """
        self.call_registered_callbacks(lnsp.PT_DN_TIMESYNC, xtime=xtime, gpstime=gpstime)
        ...
        ##CC TO REVIEW
        ##TODO check dn_frame format
        self.send_message(lnsp.PT_TIMESYNC, msg)
        
    def send_lgtw_tymesync_replay(self, txtime, gpstime):
    ...
    """ Remote Shell and Remote commands """        
    ...
    def lgtw_new_state(self, old_state, new_state):
        ...
        """ Call registered callbacks """
        self.call_registered_callbacks(lnsp.NEW_STATE, old_state=old_state.value, new_state=new_state.value)
