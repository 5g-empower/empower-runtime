...
def lenddevs_by_DevAddr(self, lenddevs):
        """Return lGTWs in this project."""
        #return self.context.lenddevs  # from project context        
        output = {}
        for eui in lenddevs.keys():
            DevAddr = lenddevs[eui].get("DevAddr")
            if DevAddr:
                DevAddr = DevAddr.capitalize()
                output[DevAddr] = lenddevs[eui]
                output[DevAddr]["DevEUI"] = eui.capitalize()
        return output ##TODO change with context
