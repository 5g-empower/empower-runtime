...
class WiFiStatsPoller(EmpowerApp):
    ...
    def wtp_up_callback(self, wtp):
        ...
        for block in wtp.supports:
            ...
            self.wifistats(block=block, every=self.every,
                           callback=self.wifi_stats_callback)

    def wifi_stats_callback(self, stats):
        """ New stats available. """

        self.log.info("New stats received from %s" % stats.block)


def launch(tenant_id, every=DEFAULT_PERIOD):
    """ Initialize the module. """

    return WiFiStatsPoller(tenant_id=tenant_id, every=every)
